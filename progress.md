...
2026-07-07 InfernoDrift5: RiftForge Online upgrade pass (Claude session):

- Rebranded the product to InfernoDrift5: RiftForge Online across index.html, manifest, package (infernodrift5 5.0.0), sw.js, build log, welcome popup, and overlay hero, with build token `20260707-id5-riftforge-v1`.
- Moved all local saves to the `infernoDrift5.*` namespace with an id5, id4, id3 read chain and id5-only writes, so ID4.1 progress migrates on first load and is never erased; secondary keys read id4 fallbacks; account cache prefix intentionally unchanged.
- Shipped the Heatline Campaign: 26 nodes across 5 worlds in the new pure module `id5-systems.js`, per-node overrides merged into the live mode definition, medals that never downgrade, first-clear rewards paid once through awardXP, Next Heat result chaining, a finale that routes to RiftForge, a new Heatline menu tab, and a Continue Heatline button on the first screen.
- Shipped RiftForge Builder: new Forge tab with palette, piece editing, undo, canvas preview, validation, saved builds, and versioned checksummed ID5-RIFT share codes; Test Drive launches real modes, generates actual drivable custom race tracks from Start plus Gates plus Finish, and spawns live boost pads, ramps, and barriers, with remaining piece types honestly labeled preview-only.
- Added the game's first audio system: procedural WebAudio engine and drift loops plus 25 one-shot events in `id5-audio.js`, wired through boost, pads, jumps, landings, collisions, markers, goals, bowling countdown, lava, level-ups, results, and UI, with persisted volume settings in a new Settings Audio section.
- Driving polish: stuck-state watchdog with warning and gentle auto-nudge, and a camera anti-snap clamp with telemetry.
- Menu revamp: tab bar grouped into Play, Build, Driver, Online workspaces; new styles for the heatline board, forge editor, and phone widths.
- Extended `__infernodriftTestApi` and `render_game_to_text` with id5 heatline, riftforge, and audio diagnostics; updated smoke_games.mjs title assertions to the ID5 strings; added id5-systems.js and id5-audio.js to the static build.
- New docs: ID5_BUG_AUDIT.md, ID5_HEATLINE_DESIGN.md, ID5_RIFTFORGE_REPORT.md, ID5_FIREBASE_ONLINE_REPORT.md, ID5_TEST_RESULTS.md, ID5_IMPLEMENTATION_REPORT.md; 15 new unit tests in tests/id5-systems.test.mjs.
- Validation in this sandbox: `node --check script.js`, `npm run typecheck`, `npm test` (65 passing), `npm run build:web`, plus full-campaign and forge round-trip simulations; browser and mobile smokes pending on a real machine because Playwright binaries cannot download here; Firebase code untouched pending Amanda's CLI auth for the rules deploy; nothing committed, pushed, or deployed.
- Follow-up in the same session: disabled the automatic push trigger on .github/workflows/sync-peer.yml per the ID5 spec, so ID5 changes can never peer-sync into the legacy InfernoDrift4 repo; manual workflow_dispatch runs remain available.
