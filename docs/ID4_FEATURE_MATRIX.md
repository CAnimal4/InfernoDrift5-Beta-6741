...
This matrix reflects the current React/TypeScript/Three worktree after parent verification on 2026-05-15. Automated verification passed for typecheck, unit tests, build, desktop gameplay smoke, mobile/dev smoke, local-online smoke, and Worker dry-run/types. Some rows still remain `partial`, `pending`, or `blocked` because the feature is not complete even though the build is playable.
...
| --- | ------------------------------------------------------------ | -------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| 1   | Better camera lean/shake/ramp anticipation                   | partial        | `client/src/game/GameCanvas.tsx`, settings sliders in `App.tsx` | verified by browser smoke; deeper tuning remains                                  |
...
| 8   | Different hunter personalities                               | source-present | `BotPersonality` and mode bot roles                             | game-core tests pending                                                           |
| 9   | Boss hunters at finales                                      | source-present | boss mode bot personality                                       | game-core tests pending                                                           |
| 10  | Varied level objectives                                      | source-present | `MODE_META`, objective update functions                         | game-core tests pending                                                           |
| 11  | More readable ramps                                          | partial        | objective rings/markers in Three scene                          | visual smoke pending                                                              |
| 12  | Landing grades/boosts                                        | source-present | landing grade state and tests                                   | verified by game-core tests                                                       |
| 13  | Backflips/jumps matter                                       | source-present | input frame and landing/trick scoring                           | game-core tests pending                                                           |
...
| 17  | Stronger world identities                                    | partial        | mode labels/objectives, shared arena                            | visual/content pass pending                                                       |
...
| 21  | Garage preview scene                                         | source-present | `GaragePreview.tsx`                                             | visual smoke pending                                                              |
| 22  | Multiple saved loadouts                                      | pending        | not observed in current React source                            | implementation pending                                                            |
| 23  | Car classes/handling                                         | source-present | `CAR_CLASSES` and player class state                            | game-core tests pending                                                           |
...
| 35  | Better pause menu                                            | source-present | menu overlay/tabs                                               | browser smoke pending                                                             |
...
| 44  | Better menu hierarchy                                        | source-present | Play/Garage/Progress/Online/Settings/How To tabs                | browser smoke pending                                                             |
...
| 10  | Visual style upgrade                     | source-present                 | Three playfield, HUD/radar CSS                                           | screenshot verification pending                                |
...
| 16  | Real garage                              | source-present                 | `GaragePreview.tsx` and class/paint/accent controls                      | visual smoke pending                                           |
...
| Better graphics/effects                       | source-present                 | Three renderer, trails, FOV, fog/lights                    | verified by screenshot review; deeper art pass remains |
