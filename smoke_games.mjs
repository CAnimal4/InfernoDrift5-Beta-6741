...
assert.equal(initialUi.product, "InfernoDrift5: RiftForge Online");
...
assert.equal(await page.locator("#release-welcome-title").textContent(), "Welcome to InfernoDrift5: RiftForge Online");
assert.equal(
...
const garageBuyState = await page.evaluate(() => {
  ...
  window.__infernodriftTestApi.applySavePayloadForTest(
    {
      saveMeta: { updatedAtMs: Date.now(), progressionUpdatedAtMs: Date.now() },
      progressionV2: {
        xp: 12000,
        totalXp: 12000,
        embers: 1500,
