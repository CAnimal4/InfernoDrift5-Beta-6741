# InfernoDrift5: RiftForge Online

Drive the heat. Build the chaos. Challenge everyone.

InfernoDrift5 is the complete evolution of the InfernoDrift4.1 launch surface: a 26-heat Heatline campaign across five worlds, the RiftForge challenge builder with shareable `ID5-RIFT` codes, full procedural game audio, driving and camera polish, and the whole 15-mode roster, all still shipping as the same root static `index.html`, `script.js`, and `style.css` experience. ID4.1 local saves migrate automatically and are never overwritten. See `ID5_IMPLEMENTATION_REPORT.md` for the current state and `ID5_CHANGE_SPEC_FOR_NEW_AGENT.md` for the change contract.

## ID4.1 baseline notes

InfernoDrift4.1 was a polish-and-upgrade release of the restored InfernoDrift4 launch surface, preserving the proven InfernoDrift4 feel instead of replacing it with the rejected React rewrite.
