...
import { createFirebaseOnlineService } from "./firebase-online.js?v=20260707-id5-riftforge-v1";
import {
  ID5_PRODUCT_NAME,
  ID5_PRODUCT_SHORT,
  ID5_SAVE_KEYS,
  ID5_LEGACY_SAVE_CHAIN,
  ID5_LEGACY_KEY_FALLBACKS,
  ...
  RIFT_PIECE_TYPES,
  RIFT_LIMITS,
  getRiftChallengeType,
  createStarterRiftChallenge,
  createRiftPiece,
  normalizeRiftChallenge,
  validateRiftChallenge,
  encodeRiftShareCode,
  decodeRiftShareCode,
  riftChallengeToRuntime,
  sanitizeLabel,
} from "./id5-systems.js?v=20260707-id5-riftforge-v1";
import { createId5Audio } from "./id5-audio.js?v=20260707-id5-riftforge-v1";

const PRODUCT_NAME = ID5_PRODUCT_NAME;
const PRODUCT_SHORT_NAME = ID5_PRODUCT_SHORT;
const CLIENT_BUILD_ID = "20260707-id5-riftforge-v1";
...
const startHeatlineBtn = document.getElementById("start-heatline-btn");
...
const SAVE_STORAGE_KEY = ID5_SAVE_KEYS.save;
// Account saves stay on the ID4 prefix on purpose: cloud/account progress is
// the source of truth and renaming the cache could orphan trusted local copies.
...
// ID4.1 and ID3 local saves stay readable. Loads walk this chain in order and
// writes always go to the ID5 key, so old saves migrate without being erased.
const LEGACY_SAVE_STORAGE_KEYS = ID5_LEGACY_SAVE_CHAIN.slice(1);
const ONLINE_STORAGE_KEY = ID5_SAVE_KEYS.online;
const FEEDBACK_STORAGE_KEY = ID5_SAVE_KEYS.feedback;
const FEEDBACK_NUDGE_STORAGE_KEY = ID5_SAVE_KEYS.feedbackNudge;
const ONBOARDING_STORAGE_KEY = ID5_SAVE_KEYS.onboarding;
const RELEASE_WELCOME_STORAGE_KEY = ID5_SAVE_KEYS.releaseWelcome;
...
const MOBILE_ROTATE_PROMPT_KEY = ID5_SAVE_KEYS.rotatePrompt;
...
function createProgressionV2() {
  ...
  return {
    ...
    heatline: normalizeHeatlineState(),
    recentRewards: [],
    rewardLog: [],
    dailyGiftSalt,
...
function normalizeProgressionV2(value = {}) {
  ...
  const next = {
    ...
    heatline: normalizeHeatlineState(source.heatline),
...
const id5Audio = createId5Audio();

function getBaseModeDefinition(mode = settings.activeGameMode) {
  return MODE_BY_ID[normalizeGameModeId(mode)] ?? MODE_BY_ID[GAME_MODE_ID33];
}

let heatlineModeDefCache = { nodeId: null, baseId: null, def: null };

function clearHeatlineModeDefCache() {
  heatlineModeDefCache = { nodeId: null, baseId: null, def: null };
}

function getActiveHeatlineNodeId() {
  try {
    return state?.heatline?.activeNodeId ?? null;
  } catch {
    return null;
  }
}

function getHeatlineModeDefinition(base, node) {
  if (
    heatlineModeDefCache.nodeId === node.id &&
    heatlineModeDefCache.baseId === base.id &&
    heatlineModeDefCache.def
  ) {
    return heatlineModeDefCache.def;
  }
  ...
  const def = {
    ...
    botCount: overrides.botCount ?? base.botCount,
    botSpeed: overrides.botSpeed ?? base.botSpeed,
    medal: node.medal ?? base.medal,
    label: node.title,
    objective: node.brief,
    reward: `Heatline first clear: +${node.firstClear.xp} XP, +${node.firstClear.embers} Embers`,
    heatlineNodeId: node.id,
  };
  heatlineModeDefCache = { nodeId: node.id, baseId: base.id, def };
  return def;
}

let riftModeDefCache = { specId: null, baseId: null, def: null };

function clearRiftModeDefCache() {
  riftModeDefCache = { specId: null, baseId: null, def: null };
}

function getRiftTestDriveSpec() {
  try {
    const spec = state?.riftforge?.testDrive;
    if (!spec) return null;
    if (normalizeGameModeId(settings.activeGameMode) !== spec.modeId) {
      return null;
    }
    return spec;
  } catch {
...
function getRiftModeDefinition(base, spec) {
  if (
    riftModeDefCache.specId === spec.id &&
    riftModeDefCache.baseId === base.id &&
    riftModeDefCache.def
  ) {
    return riftModeDefCache.def;
  }
  const type = getRiftChallengeType(spec.typeId);
  const gateTarget =
    (spec.gates?.length ?? 0) + (spec.finish?.length ? 1 : 0);
  const def = {
    ...base,
    label: `RiftForge: ${spec.title}`,
    objective: `Custom ${type?.label ?? "challenge"} test drive. ${base.objective}`,
    target: base.scene === "track" && gateTarget >= 2 ? gateTarget : base.target,
    reward: "RiftForge test drive data",
    riftTestDriveId: spec.id,
  };
  riftModeDefCache = { specId: spec.id, baseId: base.id, def };
  return def;
}

function getModeDefinition(mode = settings.activeGameMode) {
  const base = getBaseModeDefinition(mode);
  const isActiveMode =
    normalizeGameModeId(mode) === normalizeGameModeId(settings.activeGameMode);
  const nodeId = getActiveHeatlineNodeId();
  if (nodeId && isActiveMode) {
    const node = getHeatlineNode(nodeId);
    if (node && node.modeId === base.id) {
      return getHeatlineModeDefinition(base, node);
    }
  }
  const rift = getRiftTestDriveSpec();
  if (rift && isActiveMode && rift.modeId === base.id) {
    return getRiftModeDefinition(base, rift);
  }
  return base;
}

function startHeatlineNode(nodeId) {
  const node = getHeatlineNode(nodeId);
  if (!node) return false;
  const heatline = state.progressionV2.heatline ?? normalizeHeatlineState();
  if (!isHeatlineNodeUnlocked(heatline, node.id)) {
    setEffectToast("Locked: clear the previous heat first", { pulse: 0.22 });
    return false;
  }
  state.heatline.launching = true;
  state.heatline.activeNodeId = node.id;
  clearHeatlineModeDefCache();
  try {
    setActiveGameMode(node.modeId, { save: false, reset: false });
  } finally {
    state.heatline.launching = false;
  }
  startRun(true);
  return true;
}

function getHeatlineResultRows() {
  const heatline = state.modeRun.heatline;
  if (!heatline?.nodeId) return [];
  const node = getHeatlineNode(heatline.nodeId);
  if (!node) return [];
  const rows = [["Heatline", node.title]];
  if (heatline.medal) {
    rows.push([
      "Heat Medal",
      `${heatline.medal}${heatline.firstClear ? " (first clear)" : ""}`,
    ]);
  }
  if (heatline.campaignComplete) {
    rows.push(["Campaign", "Heatline complete"]);
  } else if (heatline.advanced && heatline.nextNodeId) {
    const next = getHeatlineNode(heatline.nextNodeId);
    rows.push(["Unlocked", next ? next.title : "Next heat"]);
  }
  return rows;
}
...
function repairSpecialBadgeContaminatedProgression(
  ...
  if (shouldBlockTaintedXp) {
    ...
    recordAccountProgressDiagnostic({
      ...
      newXp: 0,
      reason:
        "blocked obsolete badge repair XP from active account state; no guessed repair is written to cloud",
      markerSource,
    });
...
function applyPersistentSavePayload(
  ...
  try {
    ...
    if (data.settings && typeof data.settings === "object") {
      ...
      if (Number.isFinite(data.settings.audioMaster))
        settings.audioMaster = THREE.MathUtils.clamp(
          Number(data.settings.audioMaster),
          0,
          1,
        );
      if (Number.isFinite(data.settings.audioEngine))
        settings.audioEngine = THREE.MathUtils.clamp(
          Number(data.settings.audioEngine),
          0,
          1,
        );
      if (Number.isFinite(data.settings.audioSfx))
        settings.audioSfx = THREE.MathUtils.clamp(
          Number(data.settings.audioSfx),
          0,
          1,
        );
...
function readLocalRaw(key) {
  // Reads the ID5 key first, then any mapped legacy ID4 key, so upgraded
  // players keep dismissed prompts and preferences without new writes.
  const hit = resolveKeyWithFallback((candidate) => {
    try {
...
function setActiveTab(tabName = "settings") {
  ...
  if (tabName === "heatline") renderHeatlinePanel();
  if (tabName === "forge") renderForgePanel();
...
function setActiveGameMode(mode, { save = true, reset = false } = {}) {
  ...
  if (!state.heatline.launching && state.heatline.activeNodeId) {
    state.heatline.activeNodeId = null;
    clearHeatlineModeDefCache();
  }
  if (!state.riftforge.launching && state.riftforge.testDrive) {
    state.riftforge.testDrive = null;
    clearRiftModeDefCache();
  }
...
function getRaceTrackLayout(mode = getModeDefinition()) {
  ...
  if (
    ...
    (rift.gates?.length ?? 0) >= 2
  ) {
    const controls = [
      { x: rift.start?.x ?? 0, z: rift.start?.z ?? -60 },
      ...rift.gates.map((gate) => ({ x: gate.x, z: gate.z })),
      ...(rift.finish?.length
        ? [{ x: rift.finish[0].x, z: rift.finish[0].z }]
        : []),
    ];
    const curve = createClosedTrackCurve(controls);
    const samples = Math.max(28, controls.length * 8);
    const path = Array.from({ length: samples }, (_, index) =>
      pointFromTrackVector(curve.getPoint(index / samples)),
    );
    const checkpointCount = Math.max(2, controls.length - 1);
...
function maybeShowFeedbackNudge() {
  ...
  try {
    alreadySeen = readLocalRaw(FEEDBACK_NUDGE_STORAGE_KEY) === "1";
  } catch {
    alreadySeen = false;
  }
...
function updatePlayer(dt) {
  ...
  if (boostActive && !state.boostAudioActive) {
    state.boostAudioActive = true;
    id5Audio.event("boost-start");
  } else if (!boostActive && state.boostAudioActive) {
    state.boostAudioActive = false;
  }
  ...
  if (
    ...
    !bowlingActive &&
    (player.maxStunTimer ?? 0) <= 0
  ) {
    state.stuckAssist.timer += dt;
  } else {
    state.stuckAssist.timer = 0;
    state.stuckAssist.warned = false;
  }
  if (state.stuckAssist.timer > 2.2 && !state.stuckAssist.warned) {
    state.stuckAssist.warned = true;
    id5Audio.event("stuck-warning");
    setEffectToast("Stuck? Reverse out, or press R for a fast reset", {
      pulse: 0.24,
    });
  }
  if (state.stuckAssist.timer > 3.6) {
    state.stuckAssist.timer = 0;
    state.stuckAssist.warned = false;
    state.stuckAssist.lastResetAt = state.elapsed;
    player.speed = -14;
    player.verticalVel = Math.max(player.verticalVel, 2.4);
    setEffectToast("Recovery nudge", { pulse: 0.2 });
  }
    ...
    if (car.position.y <= floorHeight) {
      ...
      if (!car.isBot && state.wasAirborne) {
        ...
        if (bonus > 0.2) {
          ...
          id5Audio.event(bonus > 1.4 ? "land-hard" : "land");
...
function completeModeMarker(marker, amount = 1, score = 260) {
  ...
  id5Audio.event(
    marker.kind === "ring" || marker.kind === "loop"
      ? "ring"
      : marker.kind === "zone"
        ? "zone-hold"
        : "checkpoint",
...
function awardModeProgression({ won = true, reason = "" } = {}) {
  ...
  const heatlineNodeId = getActiveHeatlineNodeId();
  if (heatlineNodeId) {
    const heatlineOutcome = applyHeatlineResult(
      state.progressionV2.heatline,
      heatlineNodeId,
      { won, score, now: new Date().toISOString() },
    );
    state.progressionV2.heatline = heatlineOutcome.state;
    if (heatlineOutcome.reward) {
      const node = getHeatlineNode(heatlineNodeId);
      awardXP("heatline", heatlineOutcome.reward.xp, {
        embers: heatlineOutcome.reward.embers,
        modeId: mode.id,
        label: `Heatline: ${node?.title ?? heatlineNodeId}`,
        medal: heatlineOutcome.medal ?? "",
        reward: "Heatline first clear bonus",
      });
    }
    state.modeRun.heatline = {
      nodeId: heatlineNodeId,
      medal: heatlineOutcome.medal,
      firstClear: heatlineOutcome.firstClear,
      advanced: heatlineOutcome.advanced,
      nextNodeId: heatlineOutcome.nextNodeId,
      campaignComplete: heatlineOutcome.campaignComplete,
      improvedBest: heatlineOutcome.improvedBest,
    };
    if (heatlineOutcome.medal) id5Audio.event("medal");
  }
  savePersistentState();
  renderDailyGiftNotice();
...
function completeModeRun() {
  ...
  if (heatline?.nodeId) {
    const node = getHeatlineNode(heatline.nodeId);
    id5Audio.event(heatline.campaignComplete ? "level-up" : "run-complete");
    if (heatline.campaignComplete && !heatline.nextNodeId) {
      showMessage(
        "Heatline Complete",
        `${node?.title ?? "Final heat"} cleared with a ${medal} medal, +${xpGained} XP and +${embersGained} Embers. The whole Heatline is yours. RiftForge is open: go build a challenge the campaign never saw.`,
        "Open RiftForge",
        "heatline-forge",
    ...
    if (heatline.nextNodeId) {
      const next = getHeatlineNode(heatline.nextNodeId);
      showMessage(
        `${node?.title ?? mode.label} Complete`,
        `${medal} medal earned, +${xpGained} XP and +${embersGained} Embers.${heatline.firstClear ? " First clear bonus banked." : ""} Next heat: ${next?.title ?? "the next node"}.`,
        "Next Heat",
        "heatline-next",
        getModeResultRows(true),
      );
...
function failModeRun(
  ...
  id5Audio.event("run-failed");
  const heatlineHint = state.modeRun.heatline?.nodeId
    ? " Your Heatline best score is saved; clear the objective to advance."
    : "";
  showMessage(
    `${mode.label} Failed`,
    `${reason} Fast retry is ready; the reward preview stays visible.${heatlineHint}`,
    "Retry",
    "retry",
...
function updateBoostBowlingMode(dt) {
  ...
  if (bowling.countdown > 0) {
    const prevCount = Math.ceil(bowling.countdown);
    bowling.countdown = Math.max(0, bowling.countdown - dt);
    const nextCount = Math.ceil(bowling.countdown);
    if (nextCount < prevCount) {
      id5Audio.event(bowling.countdown <= 0 ? "go" : "countdown");
    }
...
function updateCamera(dt) {
  ...
  const cameraPrevFrame = camera.position.clone();
  camera.position.lerp(
    desired,
    dt * THREE.MathUtils.lerp(3.6, 5.2, speedRatio),
  );
  if (dt > 0) {
    const cameraStep = camera.position.distanceTo(cameraPrevFrame);
    const maxCameraStep = (Math.abs(player.speed) + 46) * dt * 2.2;
    if (cameraStep > maxCameraStep && cameraStep < 60) {
      camera.position.copy(
        cameraPrevFrame.lerp(camera.position, maxCameraStep / cameraStep),
      );
      state.cameraTelemetry.clampedSteps =
        (state.cameraTelemetry.clampedSteps ?? 0) + 1;
    }
  }
...
function handlePlayerHit(sourceBotId = -1) {
  ...
  if (state.shield > 0.2) {
    ...
    id5Audio.event("collision", { strength: 0.5 });
  } else {
    loseLife();
    state.lastFailReason =
    ...
    id5Audio.event("collision", { strength: 0.95 });
...
function hasSeenReleaseWelcome() {
  ...
  try {
    return readLocalRaw(RELEASE_WELCOME_STORAGE_KEY) === "1";
  } catch {
    return false;
  }
}
...
function applyRiftTestDrivePieces() {
  const spec = getRiftTestDriveSpec();
  if (!spec || isMaxMode()) return;
  const sizeKind = (size) => (size >= 3 ? "titan" : size === 2 ? "mega" : "normal");
  (spec.boosts ?? []).forEach((piece) => spawnExtraBoostPad(piece.x, piece.z));
  (spec.ramps ?? []).forEach((piece) => {
    const ramp = makeRamp(sizeKind(piece.s ?? 1));
    ramp.position.set(piece.x, 0, piece.z);
    ramps.push(ramp);
  });
  (spec.barriers ?? []).forEach((piece) => {
    const rot = (((piece.r ?? 0) % 180) + 180) % 180;
    const sideways = rot >= 45 && rot < 135;
    const length = 10 + (piece.s ?? 1) * 8;
    makeBarrier(piece.x, piece.z, sideways ? length : 3, sideways ? 3 : length);
  });
  if (spec.start && getModeDefinition().scene !== "track") {
    player.setPosition(spec.start.x, 1.2, spec.start.z);
    player.prevPosition.copy(player.position);
    player.velocity.set(0, 0, 0);
    player.speed = 0;
  }
  setEffectToast(`Test Drive: ${spec.title}`, { pulse: 0.3 });
}

function getRiftResultRows() {
  const spec = getRiftTestDriveSpec();
  if (!spec) return [];
  return [["RiftForge", spec.title]];
}
...
function getMobileRotatePromptDismissed() {
  ...
  try {
    return readLocalRaw(MOBILE_ROTATE_PROMPT_KEY) === "1";
  } catch {
    return false;
  }
}
...
function readOnboardingState() {
  ...
  let hasSave = false;
  try {
    hasSave = [SAVE_STORAGE_KEY, ...LEGACY_SAVE_STORAGE_KEYS].some((key) => {
      try {
...
function updateStartHeatlineButton() {
  if (!startHeatlineBtn) return;
  const summary = summarizeHeatline(state.progressionV2.heatline);
  const current = getHeatlineNode(summary.currentNodeId);
  startHeatlineBtn.textContent = summary.campaignComplete
    ? "Heatline: Replay Final Heat"
    : summary.completed > 0
      ? `Continue Heatline: ${current?.title ?? "Next Heat"}`
      : "Start Heatline Campaign";
}

function startHeatlineFromOverlay() {
  const summary = summarizeHeatline(state.progressionV2.heatline);
  const nodeId = summary.campaignComplete
    ? HEATLINE_NODES[HEATLINE_NODES.length - 1].id
    : summary.currentNodeId || getFirstHeatlineNodeId();
  const node = getHeatlineNode(nodeId);
  if (!node) return;
  const heatline = state.progressionV2.heatline ?? normalizeHeatlineState();
  if (!isHeatlineNodeUnlocked(heatline, node.id)) return;
  state.heatline.launching = true;
  state.heatline.activeNodeId = node.id;
  clearHeatlineModeDefCache();
  try {
  ...
  setStartAccountStatus(`Heatline: ${node.title}. ${node.brief}`);
...
function loadForgeState() {
  if (forgeState.loaded) return;
  const stored = readLocalJson(ID5_SAVE_KEYS.riftforge, {});
  const draft = normalizeRiftChallenge(stored?.draft);
  forgeState.draft = draft ?? createStarterRiftChallenge("race-route");
  forgeState.saved = (Array.isArray(stored?.saved) ? stored.saved : [])
    .map((entry) => normalizeRiftChallenge(entry))
    .filter(Boolean)
    .slice(0, 20);
  forgeState.loaded = true;
}

function saveForgeState() {
  writeLocalJson(ID5_SAVE_KEYS.riftforge, {
    draft: forgeState.draft,
    saved: forgeState.saved,
  });
}

function pushForgeUndo() {
  forgeState.undoStack.push(JSON.stringify(forgeState.draft));
  if (forgeState.undoStack.length > 24) forgeState.undoStack.shift();
}

function forgeUndo() {
  const previous = forgeState.undoStack.pop();
  if (!previous) return;
  try {
    forgeState.draft = normalizeRiftChallenge(JSON.parse(previous));
  } catch {
    return;
  }
  forgeState.selectedIndex = Math.min(
    forgeState.selectedIndex,
    forgeState.draft.pieces.length - 1,
  );
  saveForgeState();
  renderForgePanel();
}

function forgeSelectedPiece() {
  loadForgeState();
  return forgeState.draft.pieces[forgeState.selectedIndex] ?? null;
}

function addForgePiece(typeId) {
  loadForgeState();
  pushForgeUndo();
  const pieces = forgeState.draft.pieces;
  const last = pieces[pieces.length - 1];
  const nextZ = last ? Math.min(RIFT_LIMITS.coordRange, last.z + 30) : 0;
  pieces.push(createRiftPiece(typeId, last ? last.x : 0, nextZ, 0, 1));
  forgeState.selectedIndex = pieces.length - 1;
  saveForgeState();
  renderForgePanel();
}

function mutateSelectedForgePiece(mutator) {
  loadForgeState();
  const piece = forgeSelectedPiece();
  if (!piece) return;
  pushForgeUndo();
  const next = mutator({ ...piece });
  forgeState.draft.pieces[forgeState.selectedIndex] = createRiftPiece(
    next.t,
    next.x,
    next.z,
    next.r,
...
function drawForgePreview(canvas) {
  if (!canvas) return;
  const ctx2d = canvas.getContext("2d");
  if (!ctx2d) return;
  const size = canvas.width;
  const range = RIFT_LIMITS.coordRange;
  const toCanvas = (value) => ((value + range) / (range * 2)) * size;
  ctx2d.clearRect(0, 0, size, size);
  ctx2d.fillStyle = "rgba(10, 18, 28, 0.92)";
  ctx2d.fillRect(0, 0, size, size);
  ctx2d.strokeStyle = "rgba(86, 233, 255, 0.14)";
  ctx2d.lineWidth = 1;
  for (let grid = -range; grid <= range; grid += 40) {
    const pos = toCanvas(grid);
    ctx2d.beginPath();
    ctx2d.moveTo(pos, 0);
  ...
  ctx2d.strokeStyle = "rgba(86, 233, 255, 0.45)";
  ctx2d.strokeRect(1, 1, size - 2, size - 2);
  const pieces = forgeState.draft?.pieces ?? [];
  const routePoints = [];
  const startPiece = pieces.find((piece) => piece.t === "start");
  if (startPiece) routePoints.push(startPiece);
  pieces.forEach((piece) => {
    if (piece.t === "gate") routePoints.push(piece);
  });
  pieces.forEach((piece) => {
    if (piece.t === "finish") routePoints.push(piece);
  });
  if (routePoints.length >= 2 && forgeState.draft?.type === "race-route") {
    ctx2d.strokeStyle = "rgba(86, 233, 255, 0.5)";
    ctx2d.lineWidth = 2;
    ctx2d.beginPath();
    routePoints.forEach((piece, index) => {
      const px = toCanvas(piece.x);
      const pz = size - toCanvas(piece.z);
      if (index === 0) ctx2d.moveTo(px, pz);
      else ctx2d.lineTo(px, pz);
    });
    const first = routePoints[0];
    ctx2d.lineTo(toCanvas(first.x), size - toCanvas(first.z));
    ctx2d.stroke();
  }
  pieces.forEach((piece, index) => {
    const px = toCanvas(piece.x);
    const pz = size - toCanvas(piece.z);
    const selected = index === forgeState.selectedIndex;
    const radius = 5 + (piece.s ?? 1) * 2;
    ctx2d.save();
    ctx2d.translate(px, pz);
    ctx2d.rotate(((piece.r ?? 0) * Math.PI) / 180);
    ctx2d.fillStyle = RIFT_PIECE_COLORS[piece.t] ?? "#ffffff";
    if (piece.t === "barrier") {
      ctx2d.fillRect(-radius * 1.6, -3, radius * 3.2, 6);
    } else if (piece.t === "gate" || piece.t === "finish") {
      ctx2d.fillRect(-radius * 1.4, -2.4, radius * 2.8, 4.8);
    } else {
      ctx2d.beginPath();
    ...
    if (selected) {
      ...
      ctx2d.lineWidth = 2;
      ctx2d.beginPath();
      ctx2d.arc(0, 0, radius + 4, 0, Math.PI * 2);
      ctx2d.stroke();
    }
    ctx2d.restore();
  });
...
function launchRiftTestDrive() {
  loadForgeState();
  const validation = validateRiftChallenge(forgeState.draft);
  if (!validation.ok) {
    renderForgePanel();
    return;
  }
  const spec = riftChallengeToRuntime(forgeState.draft);
  if (!spec) return;
  state.heatline.activeNodeId = null;
  clearHeatlineModeDefCache();
  state.riftforge.launching = true;
  state.riftforge.testDrive = spec;
  clearRiftModeDefCache();
  try {
    setActiveGameMode(spec.modeId, { save: false, reset: false });
  } finally {
    state.riftforge.launching = false;
  }
  id5Audio.event("ui-click");
  startRun(true);
}
...
function renderForgePanel() {
  ...
  if (!root) return;
  loadForgeState();
  const draft = forgeState.draft;
  const type = getRiftChallengeType(draft.type) ?? RIFT_CHALLENGE_TYPES[0];
  const validation = validateRiftChallenge(draft);
  const counts = {};
  draft.pieces.forEach((piece) => {
    counts[piece.t] = (counts[piece.t] ?? 0) + 1;
  });
  const paletteButtons = RIFT_PIECE_TYPES.map(
    (piece) => `
      <button class="ghost forge-palette-btn" data-forge-add="${piece.id}" type="button" ${
        (counts[piece.id] ?? 0) >= piece.max ? "disabled" : ""
      }>
        <i style="background:${RIFT_PIECE_COLORS[piece.id]}"></i>
        ${piece.label}
        <span>${counts[piece.id] ?? 0}/${piece.max}</span>
      </button>`,
  ).join("");
  const pieceRows = draft.pieces
    .map((piece, index) => {
      const def = RIFT_PIECE_TYPES.find((entry) => entry.id === piece.t);
      return `
        <button class="forge-piece-row${index === forgeState.selectedIndex ? " selected" : ""}" data-forge-select="${index}" type="button">
          <i style="background:${RIFT_PIECE_COLORS[piece.t]}"></i>
          <span class="forge-piece-name">${def?.label ?? piece.t}</span>
          <span class="forge-piece-meta">x${piece.x} z${piece.z} r${piece.r} s${piece.s}</span>
        </button>`;
    })
    .join("");
  const typeOptions = RIFT_CHALLENGE_TYPES.map(
    (entry) =>
      `<option value="${entry.id}" ${entry.id === draft.type ? "selected" : ""}>${entry.label}</option>`,
  ).join("");
  const savedRows = forgeState.saved
    .map(
      (entry, index) => `
      <div class="forge-saved-row">
        <span class="forge-saved-name">${entry.title}</span>
        <span class="forge-saved-meta">${getRiftChallengeType(entry.type)?.label ?? entry.type}, ${entry.pieces.length} pieces</span>
        <button class="ghost" data-forge-load="${index}" type="button">Load</button>
        <button class="ghost" data-forge-delete="${index}" type="button">Delete</button>
      </div>`,
    )
    .join("");
  root.innerHTML = `
    <div class="forge-grid">
      <div class="forge-column">
        <label class="field">
          ...
          <input id="forge-title" type="text" maxlength="${RIFT_LIMITS.maxTitle}" />
        </label>
        <label class="field">
          <span>Challenge Type</span>
          <select id="forge-type">${typeOptions}</select>
        </label>
        <p class="forge-type-hint">${type.hint} Test Drive launches the real ${getBaseModeDefinition(type.modeId).label} mode.</p>
        <div class="forge-palette">${paletteButtons}</div>
        <div class="forge-piece-list" aria-label="Placed pieces">${pieceRows || '<p class="forge-empty">No pieces yet. Add some from the palette.</p>'}</div>
        <div class="forge-piece-controls">
          <button class="ghost" data-forge-move="x:-10" type="button">&larr;</button>
          <button class="ghost" data-forge-move="x:10" type="button">&rarr;</button>
          <button class="ghost" data-forge-move="z:10" type="button">&uarr;</button>
          <button class="ghost" data-forge-move="z:-10" type="button">&darr;</button>
          <button class="ghost" data-forge-rotate="45" type="button">Rotate 45</button>
          <button class="ghost" data-forge-size="1" type="button">Size</button>
          <button class="ghost" data-forge-remove="1" type="button">Delete Piece</button>
        </div>
        <div class="forge-actions">
          <button class="ghost" id="forge-undo" type="button">Undo</button>
          <button class="ghost" id="forge-reset" type="button">Reset Starter</button>
          <button class="ghost" id="forge-save" type="button">Save Build</button>
          <button class="primary" id="forge-test" type="button" ${validation.ok ? "" : "disabled"}>Test Drive</button>
        </div>
      </div>
      <div class="forge-column">
        <canvas id="forge-preview" class="forge-preview" width="320" height="320" aria-label="Top down build preview"></canvas>
        <div class="forge-validation ${validation.ok ? "ok" : "bad"}">
          ${
            validation.ok
              ? "Ready to test drive. Live pieces right now: custom race track from Start + Gates + Finish, Boost Pads, Ramps, Barriers, and your Start spawn. Rings, zones, flags, and safe pads show on this map and land in a later runtime update."
              : validation.errors.map((entry) => `<span>${entry}</span>`).join("")
          }
        </div>
        <div class="forge-share">
          <button class="ghost" id="forge-export" type="button">Export Share Code</button>
          <textarea id="forge-code" class="forge-code" rows="3" readonly placeholder="Your ID5-RIFT share code appears here."></textarea>
          <button class="ghost" id="forge-copy" type="button">Copy Code</button>
          <textarea id="forge-import-input" class="forge-code" rows="3" placeholder="Paste an ID5-RIFT code to import."></textarea>
          <button class="ghost" id="forge-import" type="button">Import Code</button>
          <p class="forge-import-notice ${forgeState.importNoticeOk ? "ok" : "bad"}">${forgeState.importNotice}</p>
        </div>
        <div class="forge-saved">
          <h4>Saved Builds (this device)</h4>
          ${savedRows || '<p class="forge-empty">Nothing saved yet.</p>'}
        </div>
      </div>
    </div>
  `;
  const titleInput = root.querySelector("#forge-title");
  if (titleInput) {
    titleInput.value = draft.title;
    titleInput.addEventListener("input", () => {
      draft.title = sanitizeLabel(titleInput.value, RIFT_LIMITS.maxTitle);
    });
  ...
  const typeSelect = root.querySelector("#forge-type");
  if (typeSelect) {
  ...
  root.querySelectorAll("[data-forge-add]").forEach((btn) => {
    bindPressAction(btn, () => {
      id5Audio.event("ui-click");
      addForgePiece(btn.dataset.forgeAdd);
    });
  });
  root.querySelectorAll("[data-forge-select]").forEach((btn) => {
    bindPressAction(btn, () => {
      forgeState.selectedIndex = Number(btn.dataset.forgeSelect);
      renderForgePanel();
    });
  });
  root.querySelectorAll("[data-forge-move]").forEach((btn) => {
    bindPressAction(btn, () => {
      const [axis, amount] = btn.dataset.forgeMove.split(":");
      mutateSelectedForgePiece((piece) => {
        piece[axis] += Number(amount);
        return piece;
      });
    });
  });
  const rotateBtn = root.querySelector("[data-forge-rotate]");
  if (rotateBtn) {
    bindPressAction(rotateBtn, () => {
      mutateSelectedForgePiece((piece) => {
        piece.r = (piece.r + 45) % 360;
        return piece;
      });
    });
  }
  const sizeBtn = root.querySelector("[data-forge-size]");
  if (sizeBtn) {
    bindPressAction(sizeBtn, () => {
      mutateSelectedForgePiece((piece) => {
        piece.s = piece.s >= 3 ? 1 : piece.s + 1;
        return piece;
      });
    });
  }
  const removeBtn = root.querySelector("[data-forge-remove]");
  if (removeBtn) {
    bindPressAction(removeBtn, () => {
      loadForgeState();
      if (forgeState.selectedIndex < 0) return;
      pushForgeUndo();
      forgeState.draft.pieces.splice(forgeState.selectedIndex, 1);
      forgeState.selectedIndex = Math.min(
        forgeState.selectedIndex,
        forgeState.draft.pieces.length - 1,
      );
  ...
  const undoBtn = root.querySelector("#forge-undo");
  if (undoBtn) bindPressAction(undoBtn, () => forgeUndo());
  const resetBtn = root.querySelector("#forge-reset");
  if (resetBtn) {
    bindPressAction(resetBtn, () => {
      pushForgeUndo();
      forgeState.draft = createStarterRiftChallenge(draft.type);
      forgeState.selectedIndex = -1;
      saveForgeState();
      renderForgePanel();
    });
  }
  const saveBtn = root.querySelector("#forge-save");
  if (saveBtn) {
    bindPressAction(saveBtn, () => {
      loadForgeState();
      const copy = normalizeRiftChallenge(forgeState.draft);
      if (!copy) return;
      forgeState.saved = [
        copy,
        ...forgeState.saved.filter((entry) => entry.id !== copy.id),
      ].slice(0, 20);
      saveForgeState();
      id5Audio.event("reward");
      renderForgePanel();
    });
  }
  const testBtn = root.querySelector("#forge-test");
  if (testBtn) bindPressAction(testBtn, () => launchRiftTestDrive());
  const exportBtn = root.querySelector("#forge-export");
  const codeBox = root.querySelector("#forge-code");
  if (exportBtn && codeBox) {
    if (forgeState.exportCode) codeBox.value = forgeState.exportCode;
    bindPressAction(exportBtn, () => {
      const encoded = encodeRiftShareCode(forgeState.draft);
      forgeState.exportCode = encoded.ok ? encoded.code : "";
      codeBox.value = encoded.ok
        ? encoded.code
        : `Cannot export yet: ${encoded.errors[0]}`;
      id5Audio.event("ui-click");
    });
  }
  const copyBtn = root.querySelector("#forge-copy");
  if (copyBtn && codeBox) {
    bindPressAction(copyBtn, () => {
      ...
      if (navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(codeBox.value).then(finish, finish);
      } else {
        try {
          document.execCommand("copy");
        } catch {
          // Selection stays highlighted for manual copy.
        }
        finish();
      }
    });
  }
  const importBtn = root.querySelector("#forge-import");
  const importInput = root.querySelector("#forge-import-input");
  if (importBtn && importInput) {
    bindPressAction(importBtn, () => {
      const decoded = decodeRiftShareCode(importInput.value);
      if (decoded.ok) {
        pushForgeUndo();
        forgeState.draft = decoded.challenge;
        forgeState.selectedIndex = -1;
        forgeState.importNotice = `Imported "${decoded.challenge.title}".`;
        forgeState.importNoticeOk = true;
        saveForgeState();
      } else {
        forgeState.importNotice = decoded.error;
        forgeState.importNoticeOk = false;
      }
      renderForgePanel();
    });
  }
  root.querySelectorAll("[data-forge-load]").forEach((btn) => {
    bindPressAction(btn, () => {
      const entry = forgeState.saved[Number(btn.dataset.forgeLoad)];
      if (!entry) return;
      pushForgeUndo();
      forgeState.draft = normalizeRiftChallenge(entry);
      forgeState.selectedIndex = -1;
      saveForgeState();
      renderForgePanel();
    });
  });
  root.querySelectorAll("[data-forge-delete]").forEach((btn) => {
    bindPressAction(btn, () => {
      forgeState.saved.splice(Number(btn.dataset.forgeDelete), 1);
      saveForgeState();
      renderForgePanel();
    });
  });
  drawForgePreview(root.querySelector("#forge-preview"));
}

function renderHeatlinePanel() {
  const board = document.getElementById("heatline-board");
  const summaryEl = document.getElementById("heatline-summary");
  if (!board) return;
  const heatline = state.progressionV2.heatline ?? normalizeHeatlineState();
  const summary = summarizeHeatline(heatline);
  updateStartHeatlineButton();
  if (summaryEl) {
    const current = getHeatlineNode(summary.currentNodeId);
    const medalBadges = ["Inferno", "Gold", "Silver", "Bronze"]
      .map((tier) =>
        summary.medalCounts[tier]
          ? `<span class="medal-chip medal-${tier.toLowerCase()}">${summary.medalCounts[tier]} ${tier}</span>`
          : "",
      )
      .join("");
    summaryEl.innerHTML = `
      <div class="heatline-progress-track" aria-hidden="true"><i style="width:${Math.round(summary.percent * 100)}%"></i></div>
      <p class="heatline-progress-copy">
        <strong>${summary.completed}/${summary.total}</strong> heats cleared${
          summary.campaignComplete
            ? ". Campaign complete. RiftForge mastery unlocked."
            : current
              ? `. Up next: <strong>${current.title}</strong>`
              : "."
        }
      </p>
      <div class="heatline-medal-row">${medalBadges}</div>
      <button class="primary heatline-continue" id="heatline-continue" type="button">
        ${summary.campaignComplete ? "Replay Final Heat" : summary.completed ? "Continue Heatline" : "Start Heatline"}
      </button>
    `;
    const continueBtn = summaryEl.querySelector("#heatline-continue");
    if (continueBtn) {
      bindPressAction(continueBtn, () => {
        id5Audio.event("ui-click");
        startHeatlineNode(
          summary.campaignComplete
            ? HEATLINE_NODES[HEATLINE_NODES.length - 1].id
  ...
  board.innerHTML = HEATLINE_WORLDS.map((world) => {
    const nodes = HEATLINE_NODES.filter((entry) => entry.worldId === world.id);
    const done = nodes.filter((entry) => heatline.completions?.[entry.id]).length;
    const cards = nodes
      .map((entry) => {
        const unlocked = isHeatlineNodeUnlocked(heatline, entry.id);
        const completed = Boolean(heatline.completions?.[entry.id]);
        const medal = heatline.medals?.[entry.id] ?? "";
        const best = heatline.bestScores?.[entry.id] ?? 0;
        const isCurrent = heatline.currentNodeId === entry.id && !completed;
        const modeLabel = getBaseModeDefinition(entry.modeId).label;
        const stateClass = `${completed ? " done" : ""}${isCurrent ? " current" : ""}${unlocked ? "" : " locked"}`;
        return `
          <article class="heatline-node${stateClass}" data-heatline-node="${entry.id}">
            <header>
              <span class="heatline-node-mode">${modeLabel}</span>
              ${medal ? `<span class="medal-chip medal-${medal.toLowerCase()}">${medal}</span>` : ""}
            </header>
            <h4>${entry.title}</h4>
            <p>${entry.brief}</p>
            <footer>
              <span class="heatline-node-best">${best ? `Best ${best}` : `Bronze at ${entry.medal.bronze}`}</span>
              <button class="primary heatline-launch" type="button" ${unlocked ? "" : "disabled"}>
                ${completed ? "Replay" : unlocked ? "Start Heat" : "Locked"}
              </button>
            </footer>
          </article>`;
      })
      .join("");
    return `
      <section class="heatline-world">
        <div class="heatline-world-head">
          <h3>${world.name}</h3>
          <p>${world.tagline}</p>
          <span class="heatline-world-count">${done}/${nodes.length}</span>
        </div>
        <div class="heatline-world-nodes">${cards}</div>
      </section>`;
  }).join("");
  board.querySelectorAll(".heatline-node .heatline-launch").forEach((btn) => {
    bindPressAction(btn, () => {
      const nodeId = btn.closest("[data-heatline-node]")?.dataset.heatlineNode;
      if (!nodeId) return;
      id5Audio.event("ui-click");
      startHeatlineNode(nodeId);
    });
  });
}
...
function dispatchGameAction(action) {
  ...
  if (action === "heatline-next") {
    const nextId =
      state.modeRun.heatline?.nextNodeId ||
      state.progressionV2.heatline?.currentNodeId ||
      getFirstHeatlineNodeId();
    if (!startHeatlineNode(nextId)) startRun(true);
    return;
  }
  if (action === "heatline-forge") {
    message.classList.remove("show");
    state.heatline.activeNodeId = null;
    clearHeatlineModeDefCache();
    setMenuOpen(true);
...
function animate(now) {
  ...
  lastTime = now;
  stepGame(dt);
  id5Audio.setRunning(state.running && !document.hidden);
  id5Audio.updateEngine({
    speedRatio: Math.min(
      1,
      Math.abs(player.speed) / (PLAYER_MAX_SPEED * PLAYER_BOOST_SPEED_MULT),
    ),
    boostActive: Boolean(input.boost || gamepadState.boost) && state.boost > 0.05,
    driftActive: Boolean(input.drift || gamepadState.drift),
    airborne: isCarAirborne(player),
  });
...
bindPressAction(startHeatlineBtn, () => startHeatlineFromOverlay());
updateStartHeatlineButton();
...
tabButtons.forEach((button) => {
  bindPressAction(button, () => {
    ...
    id5Audio.event("ui-tab");
    setActiveTab(button.dataset.tab);
  });
});

function applyAudioSettings() {
  id5Audio.setEnabled(settings.audioEnabled !== false);
  id5Audio.setVolumes({
    master: settings.audioMaster,
    engine: settings.audioEngine,
    sfx: settings.audioSfx,
  });
}

function syncAudioSettingsUi() {
  const enabledToggle = document.getElementById("audio-enabled-toggle");
  const masterRange = document.getElementById("audio-master-range");
  const engineRange = document.getElementById("audio-engine-range");
  const sfxRange = document.getElementById("audio-sfx-range");
  if (enabledToggle) enabledToggle.checked = settings.audioEnabled !== false;
  if (masterRange) masterRange.value = String(Math.round(settings.audioMaster * 100));
  if (engineRange) engineRange.value = String(Math.round(settings.audioEngine * 100));
  if (sfxRange) sfxRange.value = String(Math.round(settings.audioSfx * 100));
}

{
  const unlockId5Audio = () => {
    id5Audio.unlock();
    applyAudioSettings();
  };
  window.addEventListener("pointerdown", unlockId5Audio, { once: true });
  window.addEventListener("keydown", unlockId5Audio, { once: true });
  const enabledToggle = document.getElementById("audio-enabled-toggle");
  if (enabledToggle) {
    enabledToggle.addEventListener("change", (event) => {
      settings.audioEnabled = Boolean(event.target.checked);
      applyAudioSettings();
      savePersistentState();
      id5Audio.event("ui-click");
    });
  }
  const bindAudioRange = (id, key) => {
    const range = document.getElementById(id);
    if (!range) return;
    range.addEventListener("input", (event) => {
      settings[key] = THREE.MathUtils.clamp(
  ...
  bindAudioRange("audio-master-range", "audioMaster");
  bindAudioRange("audio-engine-range", "audioEngine");
  bindAudioRange("audio-sfx-range", "audioSfx");
  syncAudioSettingsUi();
  applyAudioSettings();
}
...
window.render_game_to_text = () => {
  const id5Diagnostics = {
    product: PRODUCT_NAME,
    build: CLIENT_BUILD_ID,
    heatline: {
      currentNodeId: state.progressionV2.heatline?.currentNodeId ?? null,
      activeNodeId: getActiveHeatlineNodeId(),
      completed: summarizeHeatline(state.progressionV2.heatline).completed,
      total: HEATLINE_NODES.length,
    },
    riftforge: {
      testDriveId: state.riftforge.testDrive?.id ?? null,
      testDriveMode: state.riftforge.testDrive?.modeId ?? null,
    },
    audio: (() => {
      const diag = id5Audio.getDiagnostics();
      return {
        supported: diag.supported,
        unlocked: diag.unlocked,
        enabled: diag.enabled,
        eventCount: diag.eventCount,
        lastEvents: diag.lastEvents.slice(-6),
      };
    })(),
  };
...
window.__infernodriftTestApi = {
  getId5State: () => ({
    ...
    build: CLIENT_BUILD_ID,
    heatline: {
      ...summarizeHeatline(state.progressionV2.heatline),
      activeNodeId: getActiveHeatlineNodeId(),
    },
    riftforge: {
      testDriveId: state.riftforge.testDrive?.id ?? null,
      testDriveMode: state.riftforge.testDrive?.modeId ?? null,
      draftPieces: forgeState.draft?.pieces?.length ?? 0,
      savedBuilds: forgeState.saved?.length ?? 0,
    },
    audio: id5Audio.getDiagnostics(),
    stuckAssist: { ...state.stuckAssist },
  }),
  startHeatlineNode: (nodeId) => startHeatlineNode(nodeId),
  applyHeatlineTestResult: ({ nodeId, won = true, score = 0 } = {}) => {
    const outcome = applyHeatlineResult(
      state.progressionV2.heatline,
      nodeId ?? state.progressionV2.heatline.currentNodeId,
      { won, score, now: new Date().toISOString() },
    );
    state.progressionV2.heatline = outcome.state;
    savePersistentState();
    return outcome;
  },
  decodeRiftCode: (code) => decodeRiftShareCode(code),
  encodeForgeDraft: () => {
    loadForgeState();
    return encodeRiftShareCode(forgeState.draft);
  },
  triggerAudioEvent: (name, params = {}) => id5Audio.event(name, params),
